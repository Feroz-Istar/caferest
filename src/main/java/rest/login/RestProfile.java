...
public class RestProfile {
	...
	public Response uploadProfile(String body) throws ParseException {
		...
		String sql = "INSERT INTO public.cafeuser ( email, password, created_at, updated_at) VALUES( '" + email
				+ "', '" + password + "', now(), NULL) ON conflict (email)  DO update set password ='"+password+"' returning id";
		// CafeeDBUtils.getInstance().insertIntoDB("");
		int id = CafeeDBUtils.getInstance().insertIntoDBWithGeneratedKey(sql);

		String profile_sql = "insert into user_profile(name, dob,user_id) values ('" + name + "','"
				+ new java.sql.Timestamp(dob1.getTime()) + "',"+id+") on conflict(user_id) do nothing";
		...
		ArrayList<HashMap<String, String>> results = CafeeDBUtils.getInstance()
				.executeQuery(Thread.currentThread().getStackTrace(), query);
